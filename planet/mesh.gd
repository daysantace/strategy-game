...
func _ready():
	var heightmap = load_heightmap("res://planet/earth/heightmap.png")
	if not heightmap:
		Logger.error("Could not load heightmap")
		return
	
	var texture = load("res://planet/earth/texture.png")
	if not texture:
		Logger.error("Could not load planet texture")
		return
...
func get_height_from_heightmap(heightmap, uv):
	var x = int(uv.x * heightmap.get_width())
	var y = int(uv.y * heightmap.get_height())
